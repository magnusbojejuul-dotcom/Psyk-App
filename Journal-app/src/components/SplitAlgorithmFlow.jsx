...
export function SplitAlgorithmFlow({ data }) {
    ...
    const renderNode = (node, pathId) => {
        ...
        return (
            ...
            >
                ...
                {/* Content Card */}
                <div className="w-full bg-white/80 backdrop-blur-sm border border-[#E8E4D9] rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-[#D9E1DA] transition-all duration-300 relative overflow-hidden isolate">
                    {/* Inner glow on hover */}
                    ...
                    {node.details && (
                        <div className="flex justify-center mt-2">
                            <div className="inline-flex items-center gap-1.5 text-[10px] font-bold text-[#839788] bg-[#F2F6F3] px-2.5 py-1 rounded-lg hover:bg-[#839788] hover:text-white transition-colors cursor-help">
                                <Info className="w-3.5 h-3.5" /> Detaljer
                            </div>
                        </div>
                    )}
                    ...
                    {node.details && (
                        <div className={`mt-3 grid transition-all duration-500 ease-in-out ${isHovered ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                            <div className="overflow-hidden">
                                <div className="pt-3 border-t border-[#E8E4D9]/60 mt-1">
                                    <h4 className="text-[10px] font-black uppercase tracking-widest text-[#839788] mb-2">Uddybende:</h4>
                                    <div className="bg-[#FAF9F6] p-3 rounded-xl border border-[#E8E4D9]/50 text-xs text-[#4A5A50] whitespace-pre-line leading-relaxed shadow-inner">
                                        {node.details}
                                    </div>
                                </div>
                            </div>
    ...
    const renderTrack = (track, trackIndex) => {
        return (
            <div key={`track-${trackIndex}`} className="flex flex-col items-center w-full min-w-[250px] flex-1 px-2 relative">
            ...
            <div className="flex flex-col items-center min-w-max md:min-w-0 w-full px-4">
                ...
                {data.tracks && data.tracks.length > 0 && (
                    <div className="relative w-full flex flex-col items-center">
                        ...
                        {data.rootNodes && data.rootNodes.length > 0 && (
                            <div className="w-full max-w-3xl hidden md:flex justify-center relative h-8 -mt-2 z-0">
                        ...
                        <div className="flex flex-col md:flex-row w-full max-w-6xl md:justify-around gap-6 md:gap-4 relative z-10">
